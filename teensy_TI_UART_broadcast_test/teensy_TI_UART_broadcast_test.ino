...
// BQ79616 frame byte definitions
#define SINGLE_READ_VAL     0x80  // single device read  (set bit 2 for broadcast)
#define SINGLE_WRITE_VAL    0x90  // single device write
...
// reg:      register address (2 bytes for BQ79616)
// data:     payload bytes
// data_len: number of payload bytes (0–8)
uint8_t build_write_frame(uint8_t type, uint8_t *buf, uint8_t dev_addr,
                              uint16_t reg, const uint8_t *data, uint8_t data_len) {
    uint8_t i = 0;
    
    // Header
    buf[i++] = type;           // frame type / direction
    if (type == SINGLE_WRITE_VAL) {
      buf[i++] = dev_addr;               // device address
    }
    buf[i++] = (uint8_t)(reg >> 8);   // register address high byte
    buf[i++] = (uint8_t)(reg & 0xFF); // register address low byte
